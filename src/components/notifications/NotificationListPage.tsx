...
export function NotificationListPage({ description }: NotificationListPageProps) {
  ...
  async function markRead(id: string) {
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Couldn't update this notification.");
    }
  }
    ...
    <div className="animate-fade-in-up">
      ...
      {!loading && !error && notifications.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-card border border-dashed border-border bg-white px-6 py-12 text-center">
          <Bell className="mb-3 h-7 w-7 text-sage-500" />
      ...
      {!loading && notifications.length > 0 && (
        <div className="space-y-3">
          {notifications.map((notification) => {
            const content = (
              <div className={`rounded-card border p-4 shadow-subtle transition-colors ${notification.read_at ? "border-border bg-white" : "border-sage-300 bg-sage-50/60"}`}>
                <div className="flex items-start gap-3">
                  <div className="rounded-input bg-white p-2 text-sage-700"><Bell className="h-4 w-4" /></div>
                  <div className="min-w-0 flex-1">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="font-medium text-ink">{notification.title}</p>
                      <span className="text-xs text-ink/45">{new Date(notification.created_at).toLocaleString()}</span>
                    </div>
                    <p className="mt-1 text-sm text-ink/60">{notification.body}</p>
                    {notification.type && <p className="mt-2 text-[11px] font-medium uppercase tracking-wide text-sage-700">{notification.type.replaceAll("_", " ")}</p>}
                  </div>
            ...
            return notification.href
              ? <Link key={notification.id} to={notification.href} onClick={() => { if (!notification.read_at) void markRead(notification.id); }}>{content}</Link>
