...
export function AgentCreateTenant() {
  const navigate = useNavigate();
  const { properties, loading: loadingProperties, refresh } = useAgentData();
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      const data = await api.postForm<{ inviteLink: string }>("/api/agent/tenants", form);
  ...
  if (inviteLink) {
    return (
      <div className="animate-fade-in-up flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <Button className="mt-6" onClick={() => navigate("/agent/tenants")}>
          Back to tenants
        </Button>
      </div>
    );
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <PageHeader title="Create tenant" description="Set up a tenant within your assigned units." />
