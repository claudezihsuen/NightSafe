...
export function AgentPaymentReview() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { getRecord, confirmPayment, rejectPayment, loading } = useAgentPayments();
    ...
    <div className="animate-fade-in-up">
      ...
      {record.receiptKey && (
        <Card className="mb-4 flex items-center justify-between gap-3">
          ...
          <a href={`${API_URL}/api/agent/payments/${record.id}/receipt`} target="_blank" rel="noreferrer">
