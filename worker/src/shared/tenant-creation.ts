...
export async function createTenantForActor(
  ...
  // R2 isn't part of the D1 transaction. Upload first, then remove the object
  // if the database batch fails so a failed tenant creation cannot leak storage.
  ...
  if (agreementFile instanceof File && agreementFile.size > 0) {
    const stored = await putValidatedFile(env, `agreements/${tenantId}`, agreementFile);
  ...
  try {
    ...
    if (agreementFileKey) {
      await env.FILES.delete(agreementFileKey).catch(() => undefined);
    }

    const message = error instanceof Error ? error.message : String(error);
    if (message.includes("UNIQUE constraint failed: leases.unit_id")) {
      return json({ error: "This unit already has an active tenant." }, 409);
    }
